...
class InterfazTableroGUI:
    ...
    def realizar_movimiento_ia(self):
        ...
        nodo_inicial.expandir_arbol(profundidad)

        # Evaluar el árbol y obtener el mejor movimiento
        Nodo.calcular_utilidad(nodo_inicial)

        # Obtener el hijo con la mayor utilidad
        mejor_hijo = max(nodo_inicial.hijos, key=lambda hijo: hijo.utilidad)
        mejor_movimiento = mejor_hijo.estado

        # Realizar el movimiento
        valor_casilla = self.juego.tablero[mejor_movimiento[0]][mejor_movimiento[1]]
        self.juego.tablero[posicion_actual[0]][posicion_actual[1]] = 0
        self.juego.tablero[mejor_movimiento[0]][mejor_movimiento[1]] = self.juego.turno_actual
        self.actualizar_puntos(self.juego.turno_actual, valor_casilla)
        self.juego.turno_actual = "c2" if self.juego.turno_actual == "c1" else "c1"
        self.dibujar_tablero()
        self.verificar_fin_juego()

        # Programar el siguiente movimiento de la IA si es necesario
        if not self.juego_terminado:
            if (self.modo_seleccionado == "IA 1 vs IA 2") or \
            (self.modo_seleccionado == "Humano vs IA" and self.juego.turno_actual == "c1"):
                self.ventana.after(500, self.realizar_movimiento_ia)
